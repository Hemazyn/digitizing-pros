...
export async function GET(request) {
  try {
    ...
    if (resourceType === "image") {
      const result = await cloudinary.search.expression(`resource_type:image AND folder:${folder}`).max_results(500).execute();
      resources = result.resources;
    } else if (resourceType === "raw" && folder === "digipros") {
      const result = await cloudinary.search.expression(`resource_type:raw AND folder:${folder}`).max_results(500).execute();
      resources = result.resources;
    } else {
