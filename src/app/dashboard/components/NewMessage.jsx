...
import { createThread } from "@/lib/chatUtils";
...
export default function NewMessage({ onClose, onCreateNewMessage }) {
  ...
  const handleSubmit = async () => {
    ...
    const newMsgObj = {
      id: threadId,
      title: subject || "Support Conversation",
      preview: input,
      ...
      <form onSubmit={handleSubmit} className="m-1 flex flex-col gap-4 rounded-lg bg-white p-3 sm:p-4">
        ...
        <div className="relative flex flex-col gap-1.5" ref={dropdownRef}>
          ...
          <button type="button" onClick={() => setDropdownOpen((prev) => !prev)} className="border-tLine flex w-full items-center justify-between gap-1.5 rounded-md border p-2.5">
            <span className="text-primary cursor-pointer text-xs font-medium">{category}</span>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder="Type message here..." rows={4} className="border-btGray w-full resize-none rounded-lg border px-3 py-2 text-sm outline-none" />
        </div>
      </form>
      {/* Footer */}
